...
/// @brief Number of atomic instructions and corresponding code line number
/// information
...
int get_line_number_from_ptx(std::string line) {
  // .loc	2 77 10, function_name $L__info_string0, inlined_at 1 18 5 ->
  // extract 18
...
std::tuple<std::unordered_map<std::string, atomic_counter>,
           std::unordered_map<std::string, std::vector<branch_counter>>>
global_mem_atomics_analysis(const std::string &filename) {
  ...
  if (file.is_open()) {
    while (std::getline(file, line)) {
      if (line.find(".visible .entry ") !=
          std::string::npos) // denotes start of the kernel
      {
        ...
        // https://cplusplus.com/reference/string/string/erase/     - erase part
        // of a string
        line.erase(
            line.begin(),
            line.begin() +
                16); // erase the first 16 character of the name of the kernel
        line.erase(
            line.end() - 1,
            line.end()); // erase the last 1 character of the name of the kernel
        kernel_name = line;
      ...
      if ((line.find(".loc") != std::string::npos) &&
          (line.find("inlined_at") != std::string::npos)) {
      ...
      if (line.find("atom.global.add") != std::string::npos) {
        ...
        counter_obj.atom_global_line_number.insert(
            std::make_tuple(code_line_number, code_line_number_raw));

        std::vector<branch_counter>::iterator last_branch_match = std::find_if(
            branch_vec.begin(), branch_vec.end(), [&](const branch_counter &i) {
      ...
      if (line.find("atom.shared.add") != std::string::npos) {
        ...
        counter_obj.atom_shared_line_number.insert(
            std::make_tuple(code_line_number, code_line_number_raw));
      ...
      if (line.substr(0, 4) ==
          "$L__") // compare the first 4 characters of the string
      {
        // clean the line
      ...
      if ((target_branch != line) && (add_to_branch_target_map == true)) {
        ...
        branch_target_line_number_map[current_target_branch] =
            branch_code_line_number;
        add_to_branch_target_map = false;
      }
      ...
      if (line.find("bra $L__") != std::string::npos) {
        ...
        if (last_branch_match != branch_vec.end()) {
          last_branch_match->target_branch_line_number =
              branch_target_line_number_map[last_branch_match->target_branch];
          if ((last_branch_match->target_branch != "") &&
              (last_branch_match->target_branch_line_number !=
               code_line_number)) {
